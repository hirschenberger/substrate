...
impl<Block: BlockT> Backend<Block> {
	...
	pub fn database_type(source: DatabaseSettingsSrc) -> ClientResult<DatabaseType> {
		...
		let _ = crate::utils::open_database::<Block>(&config, DatabaseType::Full).and_then(|_|
			return Ok(DatabaseType::Full)
		);

		let _ = crate::utils::open_database::<Block>(&config, DatabaseType::Light).and_then(|_|
			return Ok(DatabaseType::Light)
		);

		Err(sp_blockchain::Error::Backend("Unknown database type".to_owned()))
	}
