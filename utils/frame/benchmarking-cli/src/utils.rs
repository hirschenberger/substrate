...
//
// Some utility funtions shared by writer and html_writer
...
use serde::Serialize;
use std::{collections::{HashMap, HashSet}, convert::{TryFrom, TryInto}, io};
...
impl TryFrom<&BenchmarkCmd> for CmdData {
    type Error = io::Error;
    fn try_from(cmd: &BenchmarkCmd) -> Result<Self, Self::Error> {
...
pub fn string_serialize<S>(x: &u128, s: S) -> Result<S::Ok, S::Error>
where
	S: serde::Serializer,
{
